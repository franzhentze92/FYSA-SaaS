...
import { FileText, Plus, Search, Edit, Trash2, Download, Eye, ClipboardCheck, Users, Map } from 'lucide-react';
...
type TabType = 'auditoria' | 'tecnicos' | 'croquis';

const tabs: { id: TabType; label: string; icon: React.ElementType; description: string }[] = [
  { id: 'auditoria', label: 'Auditoría', icon: ClipboardCheck, description: 'Documentación para auditorías' },
  { id: 'tecnicos', label: 'Técnicos', icon: Users, description: 'Documentación técnica' },
  { id: 'croquis', label: 'Croquis', icon: Map, description: 'Croquis y planos' },
];
...
const Documentacion: React.FC = () => {
  ...
  // Get clients for admin to assign documents
  const { clientes } = useAdminServicios();

  // Fetch documents with client filtering
  const { documentos, agregarDocumento, actualizarDocumento, eliminarDocumento } = useDocumentacion(activeTab, {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Gestión de documentación del sistema
              </p>
            </div>
            {isAdmin && (
              <button
          ...
          {/* Active Tab Description & Search */}
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-2 text-gray-600">
              <activeTabInfo.icon size={18} />
              <span className="text-sm">{activeTabInfo.description}</span>
              <span className="text-xs text-gray-400">({documentos.length} documentos)</span>
            </div>
        ...
        {documentosFiltrados.length === 0 ? (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
            <activeTabInfo.icon size={48} className="mx-auto text-gray-400 mb-4" />
            <p className="text-gray-500">
              {documentos.length === 0 
                ? `No hay documentos de ${activeTabInfo.label.toLowerCase()} registrados. Agrega tu primer documento.`
                  ...
                  {isAdmin && (
                    <p className="mt-1">
                      ...
                      {documento.clienteNombre ? (
                        <span className="text-blue-600">{documento.clienteNombre}</span>
                      ) : (
                        <span className="text-gray-400">Todos (público)</span>
                      )}
                    </p>
                  )}
                </div>
